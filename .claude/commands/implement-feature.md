...
## Agents
Load **Gameplay Dev** from `.agent/agents/gameplay-dev.md` (primary, implementation).
Load **Game Designer** from `.agent/agents/game-designer.md` (game feel questions).
...
- Don't prompt about this again unless `/polish` is run
...
- Reference `docs/ASSET_RESOURCES.md` for sourcing
...
Use `unity-feature-scaffold` skill (`.agent/skills/unity-feature-scaffold/SKILL.md`) and `unity-test-runner` skill (`.agent/skills/unity-test-runner/SKILL.md`):
...
### 8.5. Code Review
Run `code-review` skill (`.agent/skills/code-review/SKILL.md`) on all new/modified files.
- In **automatic** mode: run review automatically and fix flagged issues
- In **mix/assistant** mode: ask before running
