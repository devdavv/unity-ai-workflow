...
## Agent
Load **Architect** from `.agent/agents/architect.md` (primary).
Load **Game Designer** from `.agent/agents/game-designer.md` (design validation).
...
|---|------|------|-----------|-------|--------|
| 1 | Create PlayerHealth SO | S | — | Gameplay Dev | pending |
| 2 | /implement-feature: damage system (incl. game feel) | M | 1 | Gameplay Dev + Game Designer | pending |
| 3 | /implement-feature: health UI (incl. animations) | M | 1 | UI Specialist + Game Designer | pending |
| 4 | Write health edge-case tests | S | 2 | QA Tester | pending |
```

> Game feel is embedded inside each `/implement-feature` task — not a separate row. If deferred, mark with `TODO(gamefeel)`.
